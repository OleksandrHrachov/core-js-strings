...
function removeFirstOccurrences(str, value) {
  ...
  const lastPart = str.slice(i + value.length);
...
function isPalindrome(str) {
  const reverseStr = str
    ...
    .reverse()
    .join('');
  const originStr = str.toLowerCase().replace(/[^a-z]/g, '');
...
function invertCase(str) {
  ...
  for (let i = 0; i < wordArr.length; i += 1) {
    const word = wordArr[i].split('');
    word.forEach((l, index) => {
      if (l.toUpperCase() === word[index]) {
        word[index] = l.toLowerCase();
      } else {
        word[index] = l.toUpperCase();
      }
    });
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < strArr.length; i += 1) {
    if (strArr[i].match(/[A-Z]/)) {
      strArr[i] = letters[letters.indexOf(strArr[i]) - 13].toUpperCase();
    } else if (strArr[i].match(/[a-z]/)) {
      strArr[i] = letters[letters.indexOf(strArr[i]) + 13].toLowerCase();
    }
  }
